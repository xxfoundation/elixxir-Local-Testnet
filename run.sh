...
if [[ -z ${runPermissioning} ]]; then
    GRPC_GO_LOG_VERBOSITY_LEVEL=99 GRPC_GO_LOG_SEVERITY_LEVEL=info "$BIN_PATH"/registration.binary \
     --logLevel 2 -c "$CONFIG_PATH/registration.yml" &> logs/registration_err.log &
    echo "Permissioning: " $!
else
...
if [[ -z ${runServer} ]]; then
    for i in $(seq $nodes $END); do 
        ...
        -l 2 --config "$CONFIG_PATH/servers/server-$x.yml"  &> logs/server$x\_err.log &
        echo "Server $x: " $!
    done
else
...
if [[ -z ${runGateway} ]]; then
    for i in $(seq $nodes $END); do 
        ...
        --logLevel 2 --config "$CONFIG_PATH/gateways/gateway-$x.yml" &> logs/gw$x\_err.log &
        echo "Gateway $x: " $!
    done
else
